...
function DisciplineTile({ card }: Readonly<{ card: DisciplineCard }>) {
    ...
    return (
      ...
      >
        ...
        <div className="relative">
          <div className="mb-5 inline-block rounded-pill bg-brand px-3 py-[5px] text-[11px] font-extrabold tracking-[0.06em] text-gold">
            {card.badge}
          </div>
          <h3 className="mb-3 text-[34px] font-extrabold tracking-[-0.02em]">{card.title}</h3>
  ...
  if (card.size === 'full') {
    return (
      <div className="relative flex h-full items-center justify-between gap-6 overflow-hidden rounded-[26px] bg-brand p-8 transition-transform duration-200 hover:-translate-y-0.5">
        ...
        <div className="relative">
          <div className="mb-2 text-xs font-extrabold tracking-[0.1em] text-gold">
            {card.overline}
          </div>
          <h3 className="mb-1.5 text-[26px] font-extrabold tracking-[-0.01em] text-white">
            {card.title}
          </h3>
  ...
  if (card.size === 'wide') {
    return (
      <div className="flex h-full items-center justify-between gap-5 rounded-[26px] border border-ink/10 bg-white px-[30px] py-7 shadow-card transition-colors hover:border-orange/50">
        <div>
          <div className="mb-2 text-xs font-extrabold tracking-[0.1em] text-overline">
            {card.overline}
          </div>
          <h3 className="mb-1.5 text-2xl font-extrabold tracking-[-0.01em]">{card.title}</h3>
          <p className="max-w-[340px] text-[13.5px] leading-[1.5] text-ink-soft">{card.body}</p>
        </div>
        <div className="font-display text-[54px] tracking-[-0.02em] text-cream-600">
          {card.bigLabel}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex h-full flex-col justify-between rounded-[26px] border border-ink/10 bg-white p-7 shadow-card transition-colors hover:border-orange/50">
      <div className="text-xs font-extrabold tracking-[0.1em] text-overline">{card.overline}</div>
      <div>
        <h3 className="mb-1.5 text-2xl font-extrabold tracking-[-0.01em]">{card.title}</h3>
